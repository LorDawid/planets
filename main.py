...
class Planet:
    ...
    def draw(self, planetList):
        ...
        if len(self.orbitPath) > 1:
            pygame.draw.lines(screen, tuple(color / 3 for color in self.color), False, orbitPoints, 2)

        pygame.draw.circle(screen, self.color, (self.drawPosX, self.drawPosY), self.radius / ((250 / self.AU) / SCALE))

        if self.focused:
            pygame.draw.circle(screen, (255, 0, 0), (self.drawPosX, self.drawPosY),
                               self.radius / ((250 / self.AU) / SCALE), 3)

    def calculateForce(self, planet):
        temp1 = (planet.pos[0] - self.pos[0], planet.pos[1] - self.pos[1])
        self.distance = math.sqrt(temp1[0] ** 2 + temp1[1] ** 2)

        if planet.sun:
            self.toSun = self.distance
        ...
        theta = math.atan2(temp1[1], temp1[0])
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            if event.button == 3:
                ...
                newcolor = tuple(random.randint(0, 255) for _ in range(3))
                planets[f"Planet {userPlanets + 1}"] = Planet((positionX, positionY), 10, newcolor, 5.9742 * 10 ** 24,
                                                              velocityChange[1] * 100, velX=velocityChange[0] * 100)
                userPlanets += 1
    ...
    if currentlyPressedR:
        try:
            pygame.draw.line(screen, (255, 0, 0), mousePos, pygame.mouse.get_pos())
        except AttributeError:
            pass
    ...
    try:
        highlightedPlanet = planets[highlightable[currentlyHighlighted]]
    except KeyError:
    ...
    if type(highlightedPlanet) == Planet:
        ...
        screen.blit(text, ((screenRes[0] - text.get_width()) / 2, 30))

        text = font.render(
            f"Velocity: {round(math.sqrt(highlightedPlanet.velX ** 2 + highlightedPlanet.velY ** 2) / 1000, 3)}km/s",
            True, (255, 255, 255))
    ...
    screen.blit(text, ((screenRes[0] - text.get_width()) / 2, screenRes[1] - text.get_height()))
